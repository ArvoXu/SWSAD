...
# --- Custom Imports ---
from database import init_db, get_db, Inventory, Store, Transaction, UpdateLog, Warehouse, PredictedTransaction
...
def upload_predictions():
    ...
    try:
        ...
        try:
            ...
            return jsonify({
                'error': 'JSON 格式错误，请确保文件内容是有效的 JSON 格式',
                'format_example': {
                    'expected_format': [
                        {
                            'transaction_date': '2025-08-04T00:00:00',
                            'shop_name': '商店名称',
                            'product': '商品名称',
                    ...
                    'notes': [
                        'transaction_date 必须是 YYYY-MM-DDT00:00:00 格式',
                        'shop_name 必须与库存中的商店名称完全匹配',
                        'product 必须与库存中的商品名称完全匹配',
                        'predicted_quantity 必须是非负整数'
                    ]
                }
            }), 400
        ...
        if not isinstance(predictions, list):
            return jsonify({
                'error': '数据格式错误：预测数据必须是数组/列表格式',
                'provided': type(predictions).__name__,
                'example': [{'date': '2025-08-04', 'shop': '商店名称', 'product': '商品名称', 'quantity': 5}]
            }), 400
            
        # 验证每个预测记录的格式
        required_fields = ['transaction_date', 'shop_name', 'product', 'predicted_quantity']
        for i, pred in enumerate(predictions):
            if not isinstance(pred, dict):
                return jsonify({
                    'error': f'第 {i+1} 项数据格式错误：必须是对象/字典格式',
                    'provided': type(pred).__name__,
                    'example': {'transaction_date': '2025-08-04T00:00:00', 'shop_name': '商店名称', 'product': '商品名称', 'predicted_quantity': 5}
                }), 400
                
            missing_fields = [field for field in required_fields if field not in pred]
            if missing_fields:
                return jsonify({
                    'error': f'第 {i+1} 项数据格式错误',
            ...
            # 验证日期格式
            try:
                datetime.strptime(pred['transaction_date'], '%Y-%m-%dT%H:%M:%S')
            except ValueError:
                return jsonify({'error': f'第 {i+1} 项日期格式错误：必须是 YYYY-MM-DDT00:00:00 格式'}), 400
                
            # 验证 predicted_quantity 是整数
            if not isinstance(pred['predicted_quantity'], (int, float)) or pred['predicted_quantity'] < 0:
                return jsonify({'error': f'第 {i+1} 项 predicted_quantity 字段错误：必须是非负数'}), 400
        
        db = next(get_db())
        try:
            # 清除旧的预测数据
            db.query(PredictedTransaction).delete()
            
            # 添加新的预测数据
            for pred in predictions:
                trans_date = datetime.strptime(pred['transaction_date'], '%Y-%m-%dT%H:%M:%S').replace(tzinfo=pytz.UTC)
                new_prediction = PredictedTransaction(
                    transaction_date=trans_date,
                    shop_name=pred['shop_name'],
                    product=pred['product'],
                    predicted_quantity=int(pred['predicted_quantity'])  # 确保转换为整数
                )
                db.add(new_prediction)
                
            db.commit()
            return jsonify({'message': f'成功上传 {len(predictions)} 条预测数据'})
...
@app.route('/api/predictions/<shop_name>/<product>', methods=['GET'])
def get_product_predictions(shop_name, product):
    """
    获取特定商店和产品的预测数据。
    计算预计售罄天数。
    """
    try:
        ...
        try:
            ...
            current_inventory = db.query(Inventory).filter(
                Inventory.store == shop_name,
                Inventory.product_name == product  # 使用 product_name 而不是 product
            ).first()
            ...
            predictions = db.query(PredictedTransaction).filter(
                PredictedTransaction.shop_name == shop_name,
                PredictedTransaction.product == product,
                PredictedTransaction.transaction_date >= today
            ).order_by(PredictedTransaction.transaction_date).all()
            
            # 计算预计售罄天数
            remaining_quantity = current_inventory.quantity
            days_until_sold_out = None
            
            for i, pred in enumerate(predictions):
                remaining_quantity -= pred.predicted_quantity
                if remaining_quantity <= 0:
                    days_until_sold_out = i + 1
                    break
                    
            result = {
                'current_inventory': current_inventory.quantity,
                'predictions': [{
                    'date': pred.transaction_date.strftime('%Y-%m-%d'),
                    'predicted_quantity': pred.predicted_quantity
                } for pred in predictions],
                'days_until_sold_out': days_until_sold_out
            }
...
def get_data():
    ...
    try:
        ...
        for item in inventory_items:
            ...
            today = datetime.now(pytz.UTC).date()
            predictions = db.query(PredictedTransaction).filter(
                PredictedTransaction.shop_name == item.store,
                PredictedTransaction.product == item.product_name,  # 使用 product_name 而不是 product
                PredictedTransaction.transaction_date >= today
            ).order_by(PredictedTransaction.transaction_date).all()
            
            # 添加调试日志
            logging.info(f"Getting predictions for store: {item.store}, product: {item.product_name}")
            ...
            if predictions:
                ...
                for i, pred in enumerate(predictions):
                    ...
                    if remaining_quantity <= 0 and days_until_sold_out is None:
                        days_until_sold_out = i + 1
                
                item_dict['days_until_sold_out'] = days_until_sold_out
                item_dict['predictedSales'] = predicted_sales
                logging.info(f"Calculated days_until_sold_out: {days_until_sold_out}")
                logging.info(f"Predicted sales: {predicted_sales}")
                
                # 添加预测数据的汇总信息
                total_predicted = sum(pred.predicted_quantity for pred in predictions)
                item_dict['totalPredictedQuantity'] = total_predicted
                item_dict['remainingDays'] = len(predictions)
                item_dict['averageDailyPredicted'] = round(total_predicted / len(predictions), 2) if predictions else 0
